...
def get_process_list():
    """Return a list of running processes for table output."""
    headers = ["PID", "Process name", "User / Session"]
    rows = []

    if os.name == 'nt':
        try:
            # Get processes via tasklist
            ...
            rows.append(["0", "Error retrieving process list", str(e)])
    else:
        try:
            # Get processes via ps
    ...
    # Cap the list at the first 40 processes so the screen doesn't overflow
...
def run_system_monitor():
    """Real-time CPU and RAM resource monitor."""
    clear_screen()
    theme_color = config.COLORS.get(getattr(config, 'THEME_COLOR', 'PURPLE'), config.COLORS["PURPLE"])
    reset = config.COLORS["RESET"]

    print(f"{theme_color}==================================================")
    print("         CITADEL RESOURCE MONITOR                 ")
    print(f"=================================================={reset}")
    print("\nCollecting load data (press Ctrl+C to exit)...\n")

    try:
        while True:
            if os.name == 'nt':
                # Windows data via PowerShell
                ps_cmd = (
                    "powershell -command \""
                ...
                if "|" in output:
                    ...
                    sys.stdout.write(f"\r[ HARDWARE ]: CPU: {cpu_load}% | RAM: {used_mem_p}% (Free: {free_gb} GB / {total_gb} GB)   ")
                    sys.stdout.flush()
            else:
                # Linux data
                # 1. CPU
        ...
        print(f"\n\n{config.COLORS['GRAY']}[ INFO ]: Monitor stopped.{reset}")
