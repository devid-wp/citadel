...
# run_tests.sh
#
# Citadel OS v1.0 (Core 3.0) — быстрый smoke-набор тестов.
#
# Запускается внутри chroot (или на dev-машине) непосредственно перед
# финальной сборкой ISO. Проверяет:
#   1. Импорты core/system/apps (sanity-check: нет SyntaxError / ImportError).
#   2. test_all.py — end-to-end smoke (auth, crypto, weather, geo, aliases).
#   3. pytest test-suite в tests/ — модульные тесты (35+ тестов).
#
# Использование:
#   bash run_tests.sh                # всё подряд, остановка на первой ошибке
#   bash run_tests.sh --no-pytest    # без pytest (если не установлен)
#   bash run_tests.sh --keep-going   # прогнать все этапы, даже если один упал
#
# Требует:
#   - python3 (≥ 3.11; работает на 3.14)
#   - pytest (опционально; иначе шаг pytest пропускается)
#
# Возвращает 0 при полном успехе, 1 при любом упавшем шаге.
...
if [ "$RUN_PYTEST" -eq 0 ]; then
    ...
    # Если pytest не установлен в окружении python3 — пытаемся поставить.
...
if [ "$FAIL_COUNT" -eq 0 ]; then
    ...
    if [ "$KEEP_GOING" -eq 1 ]; then
        exit 0  # явно попросили продолжить
    fi
    exit 1
fi
